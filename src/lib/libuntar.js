...
(function () {
	...
	function tarGetEntries(filename, array_buffer) {
		...
		while (offset + TAR_HEADER_SIZE < view.byteLength) {
			// Get entry name
			let entry_name = String.fromCharCode.apply(null, _tarRead(view, offset + TAR_NAME_OFFSET, TAR_NAME_SIZE)).replace(/\0/g, '');
			...
			// Get entry size
			let entry_size = parseInt(String.fromCharCode.apply(null, _tarRead(view, offset + TAR_SIZE_OFFSET, TAR_SIZE_SIZE)), 8);
			let entry_type = String.fromCharCode.apply(null, _tarRead(view, offset + TAR_TYPE_OFFSET, TAR_TYPE_SIZE)) | 0;

			// Save this as an entry if it is a file or directory
